...
public class IndividualHealthProposalService {
  ...
  public List<MedicalProposal> createIndividualHealthDtoToProposal(
      IndividualHealthInsuranceDTO individualHealthInsuranceDTO) {
    try {
  ...
  public List<MedicalProposal> convertIndividualHealthProposalDTOToProposal(
      IndividualHealthInsuranceDTO individualHealthInsuranceDTO) {
    List<MedicalProposal> medicalProposalList = new ArrayList<>();
    try {
      Optional<Branch> branchOptional =
          branchService.findById(individualHealthInsuranceDTO.getBranchId());
      Optional<Organization> organizationOptional =
          organizationService.findById(individualHealthInsuranceDTO.getOrganizationId());
      Optional<Customer> customerOptional =
          customerService.findById(individualHealthInsuranceDTO.getCustomerId());
      Optional<PaymentType> paymentTypeOptional =
          paymentTypeService.findById(individualHealthInsuranceDTO.getPaymentTypeId());
      Optional<Agent> agentOptional =
          agentService.findById(individualHealthInsuranceDTO.getAgentId());
      Optional<SalesPoints> salesPointsOptional =
          salePointService.findById(individualHealthInsuranceDTO.getSalesPointsId());
      ...
      individualHealthInsuranceDTO.getProposalInsuredPersonList().forEach(insuredPerson -> {
        ...
        medicalProposal.getMedicalProposalInsuredPersonList()
            .add(createInsuredPersonForIndividualHealth(insuredPerson));
  ...
  private MedicalProposalInsuredPerson createInsuredPersonForIndividualHealth(
      IndividualHealthProposalInsuredPersonDTO dto) {
    try {
  ...
  private MedicalProposalInsuredPersonBeneficiaries createInsuredPersonBeneficiareis(
      IndividualHealthProposalInsuredPersonBeneficiariesDTO dto) {
    try {
