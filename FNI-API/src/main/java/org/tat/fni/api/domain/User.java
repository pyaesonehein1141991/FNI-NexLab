...
@TableGenerator(name = "USER_GEN", table = "ID_GEN", pkColumnName = "GEN_NAME",
    valueColumnName = "GEN_VAL", pkColumnValue = "USER_GEN", allocationSize = 10)
@NamedQueries(value = {
    @NamedQuery(name = "User.findAll", query = "SELECT u FROM User u ORDER BY u.name ASC"),
    @NamedQuery(name = "User.findByUsercode",
        query = "SELECT u FROM User u WHERE u.usercode = :usercode"),
    @NamedQuery(name = "User.findById", query = "SELECT u FROM User u WHERE u.id = :id"),
    @NamedQuery(name = "User.changePassword",
        query = "UPDATE User u SET u.password = :newPassword WHERE u.usercode = :usercode"),
    @NamedQuery(name = "User.resetPassword",
        query = "UPDATE User u SET u.password = :defaultPassowrd WHERE u.usercode = :usercode")})
...
public class User implements Serializable {
  ...
  @JoinTable(name = "USER_BRANCH", joinColumns = @JoinColumn(name = "USERID"),
