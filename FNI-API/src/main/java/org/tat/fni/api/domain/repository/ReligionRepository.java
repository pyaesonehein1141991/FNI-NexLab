...
public interface ReligionRepository extends JpaRepository<Religion, String> {
	...
	@Query(value = "SELECT * FROM  Religion", nativeQuery = true)
	List<Object[]> findAllNativeObject();

	@Query(value = "SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Religion'", nativeQuery = true)
