...
public class ShortTermLifeProposalService {
	...
	public List<LifeProposal> createShortTermEndowmentLifeDtoToProposal(ShortTermEndowmentLifeDTO shortTermEndowmentLifeDto) {
		try {
			// convert shortTermEndowmentlifeProposalDTO to lifeproposal
			List<LifeProposal> shortTermEndowmentLifeProposalList = convertShortTermEndowmentLifeProposalDTOToProposal(shortTermEndowmentLifeDto);
	...
	public List<LifeProposal> convertShortTermEndowmentLifeProposalDTOToProposal(ShortTermEndowmentLifeDTO shortTermEndowmentLifeDto) {
		...
		try {
			...
			shortTermEndowmentLifeDto.getProposalInsuredPersonList().forEach(insuredPerson -> {
				...
				// insuredPersonInfoDTO.getAgeForNextYear(),
				// lifeProposal.getPeriodOfYears(),
				// insuredPersonInfoDTO.getIsRiskyOccupation(),
				// insuredPersonInfoDTO.getPounds(),
	...
	private ProposalInsuredPerson createInsuredPersonForShortTerm(ShortTermProposalInsuredPersonDTO dto) {
		try {
			...
			Optional<RiskyOccupation> riskyOccupationOptional = riskyOccupationService.findRiskyOccupationById(dto.getRiskoccupationID());
	...
	private InsuredPersonBeneficiaries createInsuredPersonBeneficiareis(ShortTermProposalInsuredPersonBeneficiariesDTO dto) {
		try {
			Optional<Township> townshipOptional = townShipService.findById(dto.getTownshipId());
			Optional<RelationShip> relationshipOptional = relationshipService.findById(dto.getRelationshipId());
			ResidentAddress residentAddress = new ResidentAddress();
